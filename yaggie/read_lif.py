...
class Header:
    ...
    def getSeriesHeaders(self):
        if not hasattr(self, '__seriesHeaders'):
            ...
            # I am Really tired so this can be stupid
            type_filter = lambda node: True if type(node) == xml.dom.minidom.Element else False
            headers = []
            counter = 0
            for element in root.getElementsByTagName('Element'):
                memory_node = [node for node in filter(type_filter, element.childNodes) if node.tagName=='Memory']
                if len(memory_node) > 0:
                    counter += 1
                    memory_node = memory_node[0]
                    size = memory_node.getAttribute('Size')
                    if size:
                        if int(size) > 0:
...
class Serie(SerieHeader):
    ...
    def getMetadata(self):
        ...
        # Use DimensionDescription to get voxel information
            length_x = float([d.getAttribute('Length') for d in dimension_descriptions if d.getAttribute('DimID') == '1'][0])
            length_y = float([d.getAttribute('Length') for d in dimension_descriptions if d.getAttribute('DimID') == '2'][0])
            length_z = float([d.getAttribute('Length') for d in dimension_descriptions if d.getAttribute('DimID') == '3'][0])
            number_x = float([d.getAttribute('NumberOfElements') for d in dimension_descriptions if d.getAttribute('DimID') == '1'][0])
            number_y = float([d.getAttribute('NumberOfElements') for d in dimension_descriptions if d.getAttribute('DimID') == '2'][0])
            number_z = float([d.getAttribute('NumberOfElements') for d in dimension_descriptions if d.getAttribute('DimID') == '3'][0])
